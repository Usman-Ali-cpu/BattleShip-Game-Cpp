...
void print_char(int xi, int yi, int x, int y) //  function prints a character at specific position, having four parameters
{
	...
	--y;												   // decrement of value of y
	if (xi == 0 && yi == 0)								   //  check if it is start of line
		ft_putchar('A');								   // put char 'A' to print
	else if ((xi == 0 && yi == y) || (xi == x && yi == 0)) // to check if middle of line
		ft_putchar('C');								   // put char 'C' to print
	else if (xi == x && yi == y)						   // check if it last poit character
		ft_putchar('A');								   // put char 'A' to print
...
void rush(int y, int x) // function to print rush according to given position
{
	int xi; //  declare xi for keeping point x
	int yi; // declare yi

	xi = 0;		   // initializing point of x will be start from zero
	while (xi < x) // while vlaue of xi is lesss then x
	{
