...
public class GameUI {
    ...
    public static Game getInfo() {
        ...
        System.out.println("Please enter the genre for " + name);
        String genre = in.nextLine();
        while (!g.setGenre(genre)) {
            System.out.println(
                    "Invalid genre. Valid choices are 'Puzzle', 'Shooter', or 'Arcade' \nPlease enter the genre for "
                            + name);
            genre = in.nextLine();
        }

        System.out.println("Please enter the rating for " + name);
        String rating = in.nextLine();
        while (!g.setRating(rating)) {
            System.out
                    .println("Invalid rating. Valid choices are 'E', 'T', or 'M'\nPlease enter the rating for " + name);
            System.out.println("Please enter the rating for " + name);
            rating = in.nextLine();
        }
        System.out.println("Please enter the number of players for " + name);
        int nop = in.nextInt();
        in.nextLine();
        while (!g.setNop(nop)) {
            System.out.println(
                    "Invalid number of players. This must be between 1 and 10.\nPlease enter the number of players for "
                            + name);
            nop = in.nextInt();
            in.nextLine();
        }
        System.out.println("Please enter the description for " + name);
