...
public class Account 
{
  ...
  private Vector <Transaction> transObjects;//save and manage multiple list of transaction objects
...
return "<account>" + "<accId>" + accId + "</accId>" + "<owner>" + owner + "</owner>" + "<status>" + status + "</status>" + "</account>"; 
		   ...
		   */
		  return  "<account>" + "<accId>" + accId + "</accId>" + "<owner>" + owner.toString() + "</owner>" + "<status>" + status + "</status>" + "</account>";
	  }
...
�	add transaction clone to transactions
		...
		for (int i=0; i < size; i++) { // loop through transactions to find one matching accId
			...
			if (transObjects.get(i).getTransId().equals(transaction.getTransId())) 
		{
...
�	after opening file, it needs to do something such as load the data and add to transactions list
	*/
	BufferedReader br = null;
  	try {
  		Vector <Transaction>tempObjects = new Vector<Transaction>(); // create a Vector object for transactions
  		FileInputStream fis = new FileInputStream(fileName);
        br = new BufferedReader(new InputStreamReader(fis));
        String line = null;  
        // read first line and load account level attributes
        line = br.readLine();
        accId = line.substring(line.indexOf("<accId>") + 9, line.indexOf("</accId>"));
...
�	reverse the value from active to inactive and inactive to active 
  ...
  public Account(String fileName) throws Exception
  {
	  BufferedReader br = null;
  	try {
