...
{
    double d = stod(argv[0]);               // taking first double value  from argument
    double d2 = stod(argv[1]);              // taking second double value  from argument
    auto p1 = make_pair(d, -d2);            // make a pair
    double dir = atan2(10.1 + d2, 0.0 - d); // getting value of tan
